...
import { TextField, MenuItem, Select, InputLabel, FormControl, Tab } from "@mui/material";
...
import MarksEntry, { Mark } from "@/components/MarksEntry";
...
const submitResults = async (marks: Mark[], student: Student) => {
    try {
        const response = await fetch("/api/results", {
            ...
            body: JSON.stringify({ student, scores: marks }),
        });
        ...
        toast("Results successfully submitted.");
    } catch (error) {
        console.error("Error submitting results:", error);
        toast("An error occurred. Results couldn't be submitted.");
    }
}
...
const page = () => {
    ...
    const [grading, setGrading] = useState<GradeSystem | undefined>(undefined);

    const [openMarksEntry, setOpenMarksEntry] = useState(false);
    const handleMarksEntryOpen = async (row: Student) => {

        const student = await fetchStudentById(row.id);
        if (!student) {
            toast("Failed to fetch student data for marks entry.");
            return;
        }
        setOpenMarksEntry(true)
        setEditRow(student);
    };

    const fetchGrading = async () => {
        try {
            const response = await fetch("/api/scale?name=O-Level");
            if (!response.ok) {
    ...
    const fetchStudents = async () => {
        try {
            const response = await fetch("/api/students?level=S.4");
    ...
    useEffect(() => {
        ...
        const fetchGradingData = async () => {
            const gradingData = await fetchGrading();
    ...
    return (
        <div className="p-6">
            <div className="flex justify-between items-center mb-4">
                <h2 className="text-lg font-bold mb-4">Manage Senior Four Results</h2>
                            ...
                            <TableRow key={row.id}>
                                ...
                                <TableCell>
                                    <div className="flex space-x-2">
                                        <Button
                                            size="sm"
                                            disabled={row.results?.length !== 0}
                                            variant={"secondary"}
                                            onClick={() => handleMarksEntryOpen(row)}
                                        >
                                            Add
                                        </Button>
                                        <Button
            ...
            <MarksEntry gradeSystem={grading} level={'S.4'} onSubmit={submitResults} open={openMarksEntry} setOpen={setOpenMarksEntry} student={editRow} />
